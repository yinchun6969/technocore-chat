...
VERSION="5.5.2"
SOURCE_REF="df79957c712424d55d79e276823c6666d8afb9a3"
...
BACKUP_ROOT="/root/tc-a2a-verifiable-evidence-v552-backups"
ROLLBACK="/usr/local/bin/tc-a2a-verifiable-evidence-v552-rollback"
...
[[ -f "$ENV_FILE" && -f "$CURATOR" && -f "$TELEGRAM" && -x "$ROOT/venv/bin/python" ]] || die "existing AI2AI v5 runtime not found"
[[ "$(tr -d '\0' </proc/1/comm 2>/dev/null || true)" == systemd ]] || die "AI2AI v5.5.2 requires systemd"
set -a; source "$ENV_FILE"; set +a
[[ "${AGENT_NAME:-}" == ai2ai ]] || die "v5.5.2 changes only the AI2AI Reviewer/Curator node"
id tcagent >/dev/null 2>&1 || die "tcagent user missing"

stage="$(mktemp -d /root/tc-a2a-evidence-v552.XXXXXX)"
...
"$ROOT/venv/bin/python" "$stage/patch-verified-brief-v5.5.1.py" "$TELEGRAM" \
  | grep -q 'VERIFIED_BRIEF_V551_PREFLIGHT=PASS; no writes' || die "Telegram verified-brief compatibility preflight failed"
...
grep -q 'required_functions' "$stage/patch-verified-brief-v5.5.1.py" || die "dual-layout Telegram guard missing"

echo "A2A_V552_EVIDENCE_PREFLIGHT=PASS"
echo "version=$VERSION"
echo "source_ref=$SOURCE_REF"
echo "telegram_layout=known-marked-or-unmarked;function-guarded"
echo "plan=v551-recovery,dual-layout-verified-brief,transactional-rollback"
[[ "$MODE" == apply ]] || { echo "CHECK_ONLY: no installed files, services or live state changed"; exit 0; }

exec 9>/run/lock/tc-a2a-verifiable-evidence-v552.lock
flock -n 9 || die "another v5.5.2 evidence install is running"
...
rollback_policy=transactional-code-cli-telegram-only;never-rewind-live-state-or-artifacts
EOF
...
restore_one() {
  ...
  if [[ -e "$backup/prior/$name" || -L "$backup/prior/$name" ]]; then
    ...
    echo "missing rollback metadata for $name" >&2; return 1
  fi
}
...
rollback_transaction() {
  ...
  if [[ $manage_short -eq 1 ]]; then restore_one "$SHORT_CLI" short_cli; fi
  ...
  echo "A2A_V552_TRANSACTION_ROLLBACK=COMPLETE; status=$status; live state/artifacts preserved" >&2
  exit "$status"
}
...
"$ROOT/venv/bin/python" "$stage/patch-verified-brief-v5.5.1.py" "$TELEGRAM" --apply \
  | grep -q 'VERIFIED_BRIEF_V551_PREFLIGHT=PASS; applied'
...
systemctl is-active --quiet "\$CURATOR_SERVICE"
systemctl is-active --quiet "\$TELEGRAM_SERVICE"
echo 'A2A_V552_ROLLBACK=COMPLETE; code/CLI/Telegram restored; live state and artifacts preserved'
echo "backup=\$BACKUP"
EOF
chmod 0700 "$ROLLBACK"
...
echo "A2A_V552_EVIDENCE_INSTALLED"
...
echo "brief=verified-artifacts-only;marked-and-unmarked-layout-compatible"
echo "apply_failure_policy=automatic-transaction-rollback"
