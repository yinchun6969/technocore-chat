...
CURATOR_INSTALLER="$ROOT/install-curator-reliability-v5.1.sh"

bash -n "$INSTALLER"
bash -n "$CORE_INSTALLER"
bash -n "$CURATOR_INSTALLER"
...
grep -Fq 'SOURCE_REF="ac3606b61a0730def46988151c585d5567d5d84a"' "$CURATOR_INSTALLER"
grep -Fq 'persistent-verified-stage-cache,room-read-retries,limit-200' "$CURATOR_INSTALLER"
grep -Fq 'existing_curator_state_artifacts=preserved' "$CURATOR_INSTALLER"
