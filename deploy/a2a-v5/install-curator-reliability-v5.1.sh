...
BACKUP_ROOT="/root/tc-a2a-curator-v51-backups"
SOURCE_REF="ac3606b61a0730def46988151c585d5567d5d84a"
SOURCE_URL="https://raw.githubusercontent.com/yinchun6969/technocore-chat/$SOURCE_REF/deploy/a2a-v5/autonomous-curator-v5.py"
SOURCE_SHA256="afbe8f16594187952b8d1b0bfabebfe4dfd75b1f3c101141b425ea11b1e564eb"
MODE="check"
...
stage="$(mktemp -d /root/tc-a2a-curator-v51.XXXXXX)"
...
grep -q 'ROOM_RETRIES' "$stage/autonomous-curator-v5.py" || die "room retry marker missing"
grep -q 'save_cache(workflows)' "$stage/autonomous-curator-v5.py" || die "cross-round persistence marker missing"

echo "A2A_CURATOR_V51_PREFLIGHT=PASS"
echo "source_ref=$SOURCE_REF"
echo "plan=persistent-verified-stage-cache,room-read-retries,limit-200"
...
cat >/usr/local/bin/tc-a2a-curator-v51-rollback <<EOF
#!/usr/bin/env bash
set -Eeuo pipefail
BACKUP="$backup"
CURATOR="$CURATOR"
...
echo "CURATOR_V51_ROLLED_BACK; code restored; state/cache/artifacts preserved"
echo "backup=\$BACKUP"
EOF
chmod 0700 /usr/local/bin/tc-a2a-curator-v51-rollback

systemctl restart "$SERVICE"
sleep 4
systemctl is-active --quiet "$SERVICE" || {
  systemctl --no-pager --full status "$SERVICE" || true
  die "Curator failed; run tc-a2a-curator-v51-rollback"
}

echo "A2A_CURATOR_V51_INSTALLED"
echo "service=active"
echo "existing_curator_state_artifacts=preserved"
echo "new_stage_cache=enabled"
echo "rollback=tc-a2a-curator-v51-rollback"
echo "backup=$backup"
