...
BACKUPS = Path("/root/tc-research-integrity-v554-backups")
LAUNCHER = Path("/usr/local/bin/tc-a2a-research-integrity-v554-rollback")
SERVICES = (
    "technocore-a2a-rnd-v5.service",
    "technocore-a2a-rnd-curator-v5.service",
    "technocore-a2a-telegram.service",
)
FILES = (
    BASE / "autonomous-rnd-v5.py",
    BASE / "telegram-control-v1.py",
    BASE / "research_context_v32.py",
    BASE / "human_action_center_v1.py",
    LAUNCHER,
)
...
class Services:
    ...
    def runtime_group(self):
        user = (
            self.call("systemctl", "show", SERVICES[0], "-p", "User", "--value").stdout.strip()
            or "root"
        )
    ...
    def readable(self):
        ...
        if result.returncode:
            raise RuntimeError(
                "Director user cannot read/import new module; no permissions were broadened"
            )
...
def restore(backup, root, service, require_hash=True):
    ...
    print(
        "rollback=done; only code restored; newly added module/launcher removed if absent before; all live state retained"
    )
...
def install(staging, root=Path("/"), service=None):
    ...
    spec = importlib.util.spec_from_file_location(
        "research_patcher32", staging / "repair-integrity-v554.py"
    )
    ...
    replacements = [
        patcher.director(originals[0]).encode(),
        patcher.telegram(originals[1]).encode(),
        (staging / "research_context_v32.py").read_bytes(),
        patcher.actions(targets[3].read_text()).encode(),
    ]
    for i, content in enumerate(replacements):
        compile(content, str(FILES[i]), "exec")
    if all(
        p.is_file() and p.read_bytes() == content
        for p, content in zip(targets, replacements, strict=False)
    ):
    ...
    try:
        ...
        for name, path in (
            ("director-state.json", Path("/opt/technocore-a2a/rnd-v5-state/director.json")),
        ...
        for i, (path, content) in enumerate(zip(targets, replacements, strict=True)):
            meta = manifest["files"][str(i)]
        ...
        if changed:
            ...
            for unit in stopped:
                if state[unit] == "active":
                    service.start(unit)
        raise
    print("RESEARCH_ALERT_INTEGRITY_V554_INSTALLED")
    print("backup=" + str(backup))
    print("rollback=" + str(LAUNCHER))
    print("identities/cursors/nonces/peers/service settings unchanged")
