...
def install(brain):
    ...
    if "attention score is local prioritization" not in original_prompt:
        brain.SYSTEM_PROMPT=original_prompt+"""
...
"""
    def collect_candidates(guard,cfg,guard_state):
        base=cfg["BASE"].rstrip("/"); own={cfg["NICK"],cfg["DID"]}
        ws=load_json(WORKING,{}); rows=ws.get("working_set",[]) if isinstance(ws,dict) and isinstance(ws.get("working_set"),list) else []
        amap={str(x.get("id")):x for x in rows if isinstance(x,dict) and x.get("id")}
        ...
        for p in a2a.get("peers",[]) if isinstance(a2a,dict) and isinstance(a2a.get("peers"),list) else []:
            if isinstance(p,dict) and str(p.get("did","")).startswith("did:key:"):
                try:cid=guard.peer_id(str(p["did"])); internal[cid]=p
                except Exception:pass
        room_limit=int(cfg.get("BRAIN_ROOMS","12")); base_rooms=guard.candidate_rooms(base,room_limit)
        priority=[]; deep=load_json(DEEP,{})
        ...
        candidate_limit=min(max(int(cfg.get("BRAIN_CANDIDATES","16")),8),32)
        ...
        for room in rooms:
            try:data=guard.http_json(f"{base}/r/{room}?format=json&limit={room_message_limit}")
                ...
                brain.log(f"WARN room={room} read failed: {exc}");continue
            ...
            for message in messages:
                ...
                probable=(author,seq) in clustered;cid=guard.peer_id(author);contact=guard_state.setdefault("contacts",{}).setdefault(cid,{})
                declared,likely=guard.human_signal(text,probable) if hasattr(guard,"human_signal") else (False,False)
                att=int(amap.get(cid,{}).get("attention",0) or 0)
                ...
                c={"room":room,"seq":seq,"cid":cid,"author":author,"verified":author.startswith("did:key:"),"human_self_declared":bool(declared),"likely_human_rule":bool(likely),"probable_bot_cluster":bool(probable),"hard_risk":brain.hard_risk(text),"text":text[:candidate_chars],"memory":mem,"attention":att}
                natural.append(text[:digest_chars]);old=by_cid.get(cid);rank=(att,not probable,bool(likely),bool(declared),seq)
                if old is None or rank>old["_rank"]:c["_rank"]=rank;by_cid[cid]=c
            if natural:digest[room]=natural[-digest_lines:]
        candidates=list(by_cid.values())
        familiar=[c for c in candidates if c["attention"]>=45 or c["memory"].get("relationship_stage") in {"contacted","replied","established","trusted_peer"} or c["memory"].get("internal_a2a_peer")]
        discovery=[c for c in candidates if c not in familiar]
        familiar.sort(key=lambda c:(c["attention"],c["memory"].get("replies_to_love8",0),not c["probable_bot_cluster"],c["seq"]),reverse=True)
        discovery.sort(key=lambda c:(c["human_self_declared"],c["likely_human_rule"],not c["probable_bot_cluster"],c["verified"],c["seq"]),reverse=True)
        reserve=min(max(int(cfg.get("BRAIN_DISCOVERY_RESERVE","6")),3),candidate_limit)
        fam_cap=max(0,candidate_limit-reserve);selected=familiar[:fam_cap];selected+=discovery[:candidate_limit-len(selected)]
    ...
    brain.collect_candidates=collect_candidates
    brain.VERSION="2.4.2-attention"
    return brain
