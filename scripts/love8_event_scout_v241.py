...
def save_state(d: dict[str, Any]) -> None:
    ...
    tmp = STATE.with_suffix(".tmp"); tmp.write_text(json.dumps(d, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"); tmp.chmod(0o600); os.replace(tmp, STATE)
...
def run() -> int:
    cfg = load_env(); base = cfg.get("BASE", "https://technocore.chat").rstrip("/")
    st = load_state(); cursor = int(st.get("cursor", 0) or 0)
    ...
    added = 0; max_seq = cursor
    for m in messages:
        seq = int(m.get("seq", 0) or 0); max_seq = max(max_seq, seq)
        text = str(m.get("text", "") or "").strip()
        match = CREATED_RE.match(text)
        if not match: continue
        room = match.group(1).lower()
        if not ROOM_RE.fullmatch(room): continue
        key = (seq, room)
        if key in known: continue
        rooms.append({"event_seq": seq, "room": room, "seen_at": int(time.time())}); known.add(key); added += 1
...
def status() -> int:
    st = load_state(); rooms = st.get("rooms", []) if isinstance(st.get("rooms"), list) else []
    print("===== LOVE8 EVENT SCOUT v2.4.1 =====")
    print("cursor:", st.get("cursor", 0)); print("last_run_at:", st.get("last_run_at", "-")); print("retained_rooms:", len(rooms))
    for x in rooms[-20:]:
        if isinstance(x, dict): print(f"seq={x.get('event_seq')} room={x.get('room')} seen_at={x.get('seen_at')}")
    return 0
