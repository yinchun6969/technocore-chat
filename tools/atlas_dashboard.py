...
<title>Technocore Atlas v3.3 Relay Quest</title>
<style>
...
<header class="topbar"><div class="brand"><span class="brand-mark" aria-hidden="true"></span><span class="brand-copy"><b>TECHNOCORE // PIXEL QUEST</b><small id="brand-subtitle">Atlas v3.3 · Agent 接力工作流观察器</small></span></div><div class="top-actions"><button id="language" class="lang-switch" type="button" aria-label="Switch to English">EN</button><div class="live __STATUS_CLASS__">● __STATUS_LABEL__</div></div></header>
...
const S={paused:false,pausedAt:0,sound:false,data:null,lastEvent:"",historyKey:"",workflowKey:"",started:performance.now(),replayStart:0,replayCount:0,agents:{Love8:{x:90},Aizong:{x:130},AI2AI:{x:170}},sparkles:[]};
let audio=null;
...
function applyLanguage(next,persist=true){lang=next==="en"?"en":"zh";labels=I18N[lang].labels;actions=I18N[lang].actions;const c=I18N[lang],counts=document.getElementById("footer-counts"),language=document.getElementById("language");document.documentElement.lang=lang==="zh"?"zh-CN":"en";document.title=`Technocore Atlas v3.3 ${lang==="zh"?"接力任务":"Relay Quest"}`;document.getElementById("brand-subtitle").textContent=c.subtitle;document.getElementById("game-title").textContent=c.world;document.getElementById("truth-label").textContent=c.truthLabel;document.getElementById("truth-copy").textContent=c.truth;document.getElementById("team-title").textContent=c.team;document.getElementById("workflow-title").textContent=c.workflows;document.getElementById("footer-note").textContent=c.footerNote;document.getElementById("footer-safety").textContent=c.footerSafety;counts.textContent=c.footerCounts(counts.dataset.rooms,counts.dataset.messages,counts.dataset.workflows);language.textContent=lang==="zh"?"EN":"中";language.setAttribute("aria-label",c.switchLabel);document.getElementById("replay").textContent=S.replayStart?c.relaying:c.replay;document.getElementById("sound").title=lang==="zh"?"声音默认关闭":"Sound is off by default";for(const card of document.querySelectorAll(".agent-card[data-agent]")){const description=card.querySelector("small");if(description)description.textContent=c.descriptions[card.dataset.agent]||"Agent"}S.historyKey="";if(S.data){renderHistory(S.data);document.getElementById("live-note").textContent=c.readOk(new Date().toLocaleTimeString())}else document.getElementById("live-note").textContent=c.initialNote;if(persist)try{localStorage.setItem("atlas-language",lang)}catch(_){}}
