...
<title>Technocore Atlas v3.8 Relay Quest</title>
<style>
...
Object.assign(I18N.zh,{subtitle:"Atlas v3.8 · A2A v5.4 · Agent 接力工作流观察器",truth:"3 位 Agent 作为接力小队共同移动；只有当前执行工具动作的角色代表已验证签名者，其余角色为随队与接棒动画。迷你助手与粒子仅为场景特效。",connectionOffline:"OFFLINE"});
Object.assign(I18N.en,{subtitle:"Atlas v3.8 · A2A v5.4 · Agent Relay Observer",truth:"All 3 agents travel as a relay team. Only the agent performing the tool action represents the verified signer; the other two are team-travel and handoff animation. Mini helpers and particles are visual effects only.",connectionOffline:"OFFLINE"});
...
let audio=null,musicTimer=null,musicMaster=null,musicStep=0;
...
function applyLanguage(next,persist=true){lang=next==="en"?"en":"zh";labels=I18N[lang].labels;actions=I18N[lang].actions;const c=I18N[lang],counts=document.getElementById("footer-counts"),language=document.getElementById("language"),focus=document.getElementById("focus");document.documentElement.lang=lang==="zh"?"zh-CN":"en";document.title=`Technocore Atlas v3.8 ${lang==="zh"?"接力任务":"Relay Quest"}`;document.getElementById("brand-subtitle").textContent=c.subtitle;document.getElementById("game-title").textContent=c.world;document.getElementById("truth-label").textContent=c.truthLabel;document.getElementById("truth-copy").textContent=c.truth;document.getElementById("team-title").textContent=c.team;document.getElementById("workflow-title").textContent=c.workflows;document.getElementById("footer-note").textContent=c.footerNote;document.getElementById("footer-safety").textContent=c.footerSafety;counts.textContent=c.footerCounts(counts.dataset.rooms,counts.dataset.messages,counts.dataset.workflows);language.textContent=lang==="zh"?"EN":"中";language.setAttribute("aria-label",c.switchLabel);focus.title=document.body.classList.contains("focus-mode")?c.focusExit:c.focusEnter;focus.setAttribute("aria-label",focus.title);document.getElementById("replay").textContent=S.replayStart?c.relaying:c.replay;document.getElementById("sound").title=lang==="zh"?"开启高能 8-bit 配乐":"Enable high-energy 8-bit soundtrack";for(const card of document.querySelectorAll(".agent-card[data-agent]")){const description=card.querySelector("small");if(description)description.textContent=c.descriptions[card.dataset.agent]||"Agent"}S.historyKey="";updateProgress(S.journey);if(S.data){renderHistory(S.data);updateObservation(S.data.observation)}else document.getElementById("live-note").textContent=c.initialNote;if(persist)try{localStorage.setItem("atlas-language",lang)}catch(_){}}
...
function tone(freq,when,duration,type="square",level=.16){if(!audio||!musicMaster)return;const osc=audio.createOscillator(),gain=audio.createGain();osc.type=type;osc.frequency.setValueAtTime(freq,when);gain.gain.setValueAtTime(level,when);gain.gain.exponentialRampToValueAtTime(.001,when+duration);osc.connect(gain).connect(musicMaster);osc.start(when);osc.stop(when+duration+.03)}
function kick(when){if(!audio||!musicMaster)return;const osc=audio.createOscillator(),gain=audio.createGain();osc.type="sine";osc.frequency.setValueAtTime(120,when);osc.frequency.exponentialRampToValueAtTime(42,when+.13);gain.gain.setValueAtTime(.42,when);gain.gain.exponentialRampToValueAtTime(.001,when+.16);osc.connect(gain).connect(musicMaster);osc.start(when);osc.stop(when+.18)}
function queueMusic(){if(!S.sound||!audio||!musicMaster)return;const start=audio.currentTime+.04,melodies=[[330,392,440,523,440,392,330,494],[392,440,523,659,523,494,440,392],[440,523,659,784,659,523,494,440],[392,494,587,659,587,494,440,330]],bass=[110,131,147,123][musicStep%4],notes=melodies[musicStep%melodies.length];notes.forEach((freq,i)=>tone(freq,start+i*.18,.15,i%4===3?"sawtooth":"square",.13));for(let i=0;i<4;i++){tone(bass,start+i*.36,.3,"triangle",.2);kick(start+i*.36)}musicStep++}
async function startMusic(){if(!audio)return;await audio.resume?.();musicMaster=audio.createGain();musicMaster.gain.setValueAtTime(.18,audio.currentTime);musicMaster.connect(audio.destination);musicStep=0;queueMusic();musicTimer=setInterval(queueMusic,1440)}
function stopMusic(){if(musicTimer)clearInterval(musicTimer);musicTimer=null;const oldMaster=musicMaster;if(oldMaster&&audio){oldMaster.gain.cancelScheduledValues(audio.currentTime);oldMaster.gain.setValueAtTime(Math.max(.001,oldMaster.gain.value),audio.currentTime);oldMaster.gain.exponentialRampToValueAtTime(.001,audio.currentTime+.12);setTimeout(()=>oldMaster.disconnect(),180)}musicMaster=null}
function blip(kind){if(!S.sound||!audio||!musicMaster)return;const freq={WORKFLOW_TASK:660,BUILD_RESULT:784,CHALLENGE:523,REVISED_RESULT:880,COMPLETE:1047}[kind]||600;tone(freq,audio.currentTime,.22,"square",.28)}
...
function updateObservation(info){const c=I18N[lang],badge=document.getElementById("observation-status"),note=document.getElementById("live-note"),allowed=new Set(["ok","degraded","stale","waiting"]),state=allowed.has(info?.status)?info.status:"waiting",last=info?.last_success?formatTime(info.last_success):"";badge.className="live ok";badge.textContent="● LIVE";if(state==="ok"){note.textContent=c.readOk(new Date().toLocaleTimeString());note.style.borderColor="#43d36f"}else if(state==="degraded"){note.textContent=c.readDegraded(last);note.style.borderColor="#ffd84a"}else if(state==="stale"){note.textContent=c.readStale(last);note.style.borderColor="#ff5b5b"}else{note.textContent=c.readWaiting;note.style.borderColor="#ffd84a"}}
async function refresh(){const c=I18N[lang];try{const res=await fetch(`/atlas.json?v=38&t=${Date.now()}`,{cache:"no-store",headers:{Accept:"application/json","Cache-Control":"no-cache"}});if(!res.ok)throw new Error("HTTP "+res.status);const data=await res.json(),wf=data?.snapshot?.workflows?.[0],workflowKey=wf?`${wf.task_id}:${wf.updated_at}:${(wf.stages||[]).length}`:"";S.data=data;updateHud(data);renderHistory(data);updateObservation(data.observation);if(workflowKey&&workflowKey!==S.workflowKey){S.workflowKey=workflowKey;beginReplay()}}catch(_){const badge=document.getElementById("observation-status"),note=document.getElementById("live-note");badge.className="live stale";badge.textContent=`● ${c.connectionOffline}`;note.textContent=c.readFail;note.style.borderColor="#ff5b5b"}}
...
document.getElementById("sound").addEventListener("click",async e=>{S.sound=!S.sound;if(S.sound){const Audio=window.AudioContext||window.webkitAudioContext;audio=audio||(Audio?new Audio():null);await startMusic();blip(stages().at(-1)?.kind)}else stopMusic();e.currentTarget.setAttribute("aria-pressed",String(S.sound));e.currentTarget.textContent=S.sound?"♪ ON":"♪ OFF"});
