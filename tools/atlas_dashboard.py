...
<title>Technocore Atlas v3.2 Relay Quest</title>
<style>
...
<header class="topbar"><div class="brand"><span class="brand-mark" aria-hidden="true"></span><span class="brand-copy"><b>TECHNOCORE // PIXEL QUEST</b><small>Atlas v3.2 · Agent 接力工作流观察器</small></span></div><div class="live __STATUS_CLASS__">● __STATUS_LABEL__</div></header>
...
<div class="truth"><b>图例：</b>3 位主角按“移动 → 工作 → 签名交接”依次行动；迷你助手与粒子仅为场景特效。只有已验证签名事件才进入回放。</div></section>
...
const labels={WORKFLOW_TASK:"任务分派",BUILD_RESULT:"构建执行",CHALLENGE:"审查挑战",REVISED_RESULT:"修订提交",COMPLETE:"验收升旗"};
const owners={WORKFLOW_TASK:"Love8",BUILD_RESULT:"Aizong",CHALLENGE:"AI2AI",REVISED_RESULT:"Aizong",COMPLETE:"Love8"};
const actions={WORKFLOW_TASK:"读取目标 · 校验入口 · 分派",BUILD_RESULT:"分析需求 · 兼容构建 · 签名",CHALLENGE:"研究核验 · 提出挑战 · 等待修订",REVISED_RESULT:"吸收反馈 · 安全修订 · 再次提交",COMPLETE:"核对结果 · 最终汇总 · 升起战旗"};
...
function beginReplay(){if(!stages().length)return;resetActors();S.replayStart=performance.now();S.replayCount=0;const button=document.getElementById("replay");button.setAttribute("aria-pressed","true");button.textContent="● RELAY"}
function relay(now){const all=stages();if(!S.replayStart||!all.length)return{events:all,current:null,index:Math.max(0,all.length-1),phase:"idle",progress:1,active:false};const elapsed=now-S.replayStart,index=Math.floor(elapsed/STEP_MS);if(index>=all.length){if(elapsed>all.length*STEP_MS+1400){S.replayStart=0;S.replayCount=0;const button=document.getElementById("replay");button.setAttribute("aria-pressed","false");button.textContent="▶ REPLAY"}return{events:all,current:null,index:Math.max(0,all.length-1),phase:"idle",progress:1,active:false}}const progress=(elapsed-index*STEP_MS)/STEP_MS,phase=progress<MOVE_END?"move":progress<WORK_END?"work":"handoff",committed=index+(phase==="handoff"?1:0);if(committed>S.replayCount){S.replayCount=committed;blip(all[index]?.kind)}return{events:all.slice(0,committed),current:all[index],index,phase,progress,active:true}}
...
async function refresh(){try{const res=await fetch("/atlas.json",{cache:"no-store",headers:{Accept:"application/json"}});if(!res.ok)throw new Error("HTTP "+res.status);const data=await res.json(),wf=data?.snapshot?.workflows?.[0],workflowKey=wf?`${wf.task_id}:${wf.updated_at}:${(wf.stages||[]).length}`:"";S.data=data;updateHud(data);renderHistory(data);if(workflowKey&&workflowKey!==S.workflowKey){S.workflowKey=workflowKey;beginReplay()}document.getElementById("live-note").textContent=`动态连接正常 · 最近读取 ${new Date().toLocaleTimeString()} · 上游快照约每 30 秒更新`;document.getElementById("live-note").style.borderColor="#43d36f"}catch(_){document.getElementById("live-note").textContent="动态读取暂时失败；保留最后一帧并等待自动重试。";document.getElementById("live-note").style.borderColor="#ffd84a"}}
