...
<title>Technocore Atlas v3.6 Relay Quest</title>
<style>
...
;
Object.assign(I18N.zh,{stageObserved:"已观察签名",stageAwaiting:"等待签名",stageUnobserved:"尚未观察到",stageMeaning:"绿色＝已验证签名；黄色＝等待下一条签名；灰色＝尚未观察到，不代表失败或 Agent 离线。",sourceOriginal:"签名原文（保持原语言，未改写）",verifiedSummary:(agent,stage,action)=>`可验证阶段：${agent} · ${stage} · ${action}`,waitingFor:stage=>`已签名 ${stage}，正在等待下一阶段`,signedCount:(n,next)=>`${n}/5 个签名阶段 · 等待 ${next}`,workflowStatus:(n,next)=>`${n}/5 · 等待${next}`});
Object.assign(I18N.en,{stageObserved:"Signed & observed",stageAwaiting:"Awaiting signature",stageUnobserved:"Not observed",stageMeaning:"Green = verified signature; yellow = awaiting the next signature; gray = not observed. Gray does not prove failure or agent downtime.",sourceOriginal:"Signed original (source language, unchanged)",verifiedSummary:(agent,stage,action)=>`Verified stage: ${agent} · ${stage} · ${action}`,waitingFor:stage=>`Signed ${stage}; awaiting the next stage`,signedCount:(n,next)=>`${n}/5 signed stages · awaiting ${next}`,workflowStatus:(n,next)=>`${n}/5 · Await ${next}`});
