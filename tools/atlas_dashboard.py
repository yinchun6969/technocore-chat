...
<title>Technocore Atlas v3.9 Relay Quest</title>
<style>
...
.live.waiting{color:var(--gold)}.live.stale{color:var(--red)}.upstream{font-size:11px;min-width:58px;text-align:center}.upstream.ok{color:#62f59c}.upstream.degraded,.upstream.waiting{color:var(--gold)}.upstream.stale{color:var(--red)}
...
html[lang="en"] .source-label,html[lang="en"] .signed-source,html[lang="en"] .event-row .truncated{display:none}
...
@media(min-width:900px){main{width:min(1240px,calc(100% - 40px));padding-top:22px}.screen{max-height:none}.game-head{padding:11px 15px}.game-title{font-size:18px}.pixel-btn{padding:8px 14px}.truth{font-size:13px;padding:10px 14px}.agent-card{padding:15px}.event-list{display:grid;grid-template-columns:repeat(2,minmax(0,1fr))}.notice{font-size:13px}.topbar{padding-left:max(20px,env(safe-area-inset-left));padding-right:max(20px,env(safe-area-inset-right))}}
@media(max-width:700px){.topbar{position:relative;padding:9px 8px}.brand{gap:7px}.brand-mark{width:27px;height:29px;flex-basis:27px}.brand b{font-size:clamp(15px,4vw,19px)}.top-actions{gap:4px}.lang-switch{min-width:36px;padding:4px}.live{padding:4px 5px}.upstream{min-width:48px;font-size:10px}main{padding-left:8px;padding-right:8px}.agent-grid{grid-template-columns:1fr}.quest-map{grid-template-columns:1fr}.quest-step{display:flex;align-items:center;gap:8px;text-align:left}.quest-step>span{margin:0}.quest-step b,.quest-step small{font-size:11px}.event-row header time{width:100%;margin:0}.game-head{align-items:flex-start}.game-title{font-size:12px}.hud{font-size:11px}.hud .hide-mobile{display:none}}
...
<header class="topbar" data-product="TECHNOCORE // PIXEL QUEST"><div class="brand"><svg class="brand-mark" viewBox="0 0 100 132" role="img" aria-label="Technocore flag logo"><path class="logo-white" d="M0 0 20 17H100V98L80 98H0Z"/><path class="logo-cut" d="M20 35H80V52H60V82H40V52H20ZM0 63H40V66H0ZM80 63H100V66H80Z"/><path class="logo-cyan" d="M80 98 100 82V132Z"/></svg><span class="brand-copy"><b>technocore</b><small id="brand-subtitle">Atlas v3.9 · A2A v5.4 · Agent 接力工作流观察器</small></span></div><div class="top-actions"><button id="language" class="lang-switch" type="button" aria-label="Switch to English">EN</button><div id="observation-status" class="live ok" title="Viewer connection">● LIVE</div><div id="upstream-status" class="live upstream waiting" title="Waiting for upstream collection">↑ WAIT</div></div></header>
...
Object.assign(I18N.zh,{subtitle:"Atlas v3.9 · A2A v5.4 · Agent 接力工作流观察器",truth:"3 位 Agent 作为接力小队共同移动；顶砖、接棒与升旗是由已验证签名阶段驱动的原创像素演出。只有当前执行工具动作的角色代表签名者。",connectionOffline:"OFFLINE",upstreamOk:"上游采集正常",upstreamRetry:n=>`上游重试中 · 连续失败 ${n} 次`,upstreamStale:"上游快照已过期",upstreamWait:"等待上游首份快照"});
Object.assign(I18N.en,{subtitle:"Atlas v3.9 · A2A v5.4 · Agent Relay Observer",truth:"All 3 agents travel as a relay team. Original brick-hit, handoff and flag-raise effects are driven by verified signed stages. Only the agent performing the tool action represents the signer.",connectionOffline:"OFFLINE",upstreamOk:"Upstream collection healthy",upstreamRetry:n=>`Upstream retrying · ${n} consecutive failures`,upstreamStale:"Upstream snapshot is stale",upstreamWait:"Waiting for the first upstream snapshot"});
...
const S={paused:false,pausedAt:0,sound:false,data:null,lastEvent:"",historyKey:"",workflowKey:"",journey:null,started:performance.now(),replayStart:0,replayCount:0,cueKey:"",agents:{Love8:{x:homes.Love8},Aizong:{x:homes.Aizong},AI2AI:{x:homes.AI2AI}},sparkles:[]};
let audio=null,audioMaster=null,musicMaster=null,sfxMaster=null,musicTimer=null,musicStep=0;
...
function applyLanguage(next,persist=true){lang=next==="en"?"en":"zh";labels=I18N[lang].labels;actions=I18N[lang].actions;const c=I18N[lang],counts=document.getElementById("footer-counts"),language=document.getElementById("language"),focus=document.getElementById("focus");document.documentElement.lang=lang==="zh"?"zh-CN":"en";document.title=`Technocore Atlas v3.9 ${lang==="zh"?"接力任务":"Relay Quest"}`;document.getElementById("brand-subtitle").textContent=c.subtitle;document.getElementById("game-title").textContent=c.world;document.getElementById("truth-label").textContent=c.truthLabel;document.getElementById("truth-copy").textContent=c.truth;document.getElementById("team-title").textContent=c.team;document.getElementById("workflow-title").textContent=c.workflows;document.getElementById("footer-note").textContent=c.footerNote;document.getElementById("footer-safety").textContent=c.footerSafety;counts.textContent=c.footerCounts(counts.dataset.rooms,counts.dataset.messages,counts.dataset.workflows);language.textContent=lang==="zh"?"EN":"中";language.setAttribute("aria-label",c.switchLabel);focus.title=document.body.classList.contains("focus-mode")?c.focusExit:c.focusEnter;focus.setAttribute("aria-label",focus.title);document.getElementById("replay").textContent=S.replayStart?c.relaying:c.replay;document.getElementById("sound").title=lang==="zh"?"开启原创 8-bit 平台游戏配乐与音效":"Enable original 8-bit platform soundtrack and effects";for(const card of document.querySelectorAll(".agent-card[data-agent]")){const description=card.querySelector("small");if(description)description.textContent=c.descriptions[card.dataset.agent]||"Agent"}S.historyKey="";updateProgress(S.journey);if(S.data){renderHistory(S.data);updateObservation(S.data.observation)}else document.getElementById("live-note").textContent=c.initialNote;if(persist)try{localStorage.setItem("atlas-language",lang)}catch(_){}}
...
function resetActors(){for(const name of Object.keys(S.agents))S.agents[name].x=homes[name];S.sparkles=[];S.cueKey=""}
...
function block(x,y,kind,done,current,bump=0){y-=bump;const c=done?"#e9972f":"#bb7130";px(x,y,58,45,c);px(x,y,58,6,done?"#ffd34f":"#dc9349");px(x,y+39,58,6,"#713e27");px(x+26,y,6,45,"#8d4d29");if(current){px(x-4,y-4,66,4,"#fff35e");px(x-4,y+45,66,4,"#fff35e")}text(kind,x+29,y+13,13,done?"#fff6ad":"#412515","center")}
...
function draw(now){const t=(now-S.started)/1000,w=960,h=540,workflow=stage(),journey=relay(now),events=journey.events,current=journey.current,last=events[events.length-1],kind=current?.kind||last?.kind||order[0],idx=Math.max(0,order.indexOf(kind)),owner=owners[kind],targets=formationTargets(owner,idx),complete=events.some(e=>e.kind==="COMPLETE"),copy=I18N[lang],workP=journey.active&&journey.phase==="work"?clamp((journey.progress-MOVE_END)/(WORK_END-MOVE_END),0,1):0,jump=workP<.48?Math.sin(clamp(workP/.48,0,1)*Math.PI)*26:0,bump=workP>.2&&workP<.42?Math.sin((workP-.2)/.22*Math.PI)*10:0,flagProgress=complete&&!journey.active?1:kind==="COMPLETE"&&journey.active?clamp((journey.progress-WORK_END)/(1-WORK_END),0,1):complete?1:0;S.journey=journey;updateProgress(journey);cueJourney(journey,kind);ctx.imageSmoothingEnabled=false;px(0,0,w,h,"#49bdf2");px(0,315,w,125,"#75d8f7");cloud((70+t*1.5)%1050-90,85,1);cloud((540+t*.85)%1100-100,120,.8);hill(-20,410,260,150,"#62c969");hill(170,410,330,205,"#3fac65");hill(560,410,330,180,"#64c96c");pipe(25,350);flag(875,410,flagProgress,t);for(let i=0;i<5;i++){const done=events.some(e=>e.kind===order[i]),stageBump=i===idx?bump:0;block(stageXs[i]-29,350,`${i+1}`,done,i===idx&&journey.active&&!done,stageBump);text(labels[order[i]],stageXs[i],322-stageBump,10,"#fff","center");if(done||stageBump>4)coin(stageXs[i],292-stageBump*2,t*2+i)}px(0,410,w,40,"#8b5a32");for(let x=0;x<w;x+=32){px(x,410,30,18,"#c3843f");px(x+15,430,30,18,"#a86835")}px(0,450,w,90,"#6d3e27");for(let x=0;x<w;x+=48)px(x,475+(x%96?20:0),25,9,"#855031");if(journey.active)for(const name of agentNames){const speed=journey.phase==="move"?.045:.12;S.agents[name].x+=(targets[name]-S.agents[name].x)*speed}for(const name of agentNames){const distance=Math.abs(targets[name]-S.agents[name].x),activeAction=journey.active&&name===owner&&((journey.phase==="work"&&workP>.46)||(kind==="COMPLETE"&&journey.phase==="handoff")),motion={walking:journey.active&&journey.phase==="move"&&distance>1,watching:journey.active&&name!==owner&&journey.phase!=="move",action:activeAction?kind:""};avatar(name,S.agents[name].x,405-(name===owner?jump:0),t,motion)}if(journey.active&&journey.phase==="handoff"&&idx<order.length-1){const nextOwner=owners[order[idx+1]];handoff(S.agents[owner].x,S.agents[nextOwner].x,335,journey.progress,colors[owner])}const hc=["#e2f14a","#f57bd7","#73f0d1","#ff9f43"];for(let i=0;i<10;i++)helper(90+i*82,405,hc[i%4],t,i);const bubbleCopy=lang==="en"?actions[kind]:(current||last)?.content;bubble(bubbleCopy,clamp(stageXs[idx]-125,12,698),190,250);if(complete&&S.sparkles.length===0)addFx();drawFx(1/60);const phaseLabel=journey.phase==="move"?copy.teamMove(owner):journey.phase==="work"?actions[kind]:journey.phase==="handoff"?(idx<4?copy.handoff(owner,owners[order[idx+1]]):copy.accepted):complete?"QUEST CLEAR!":workflow?copy.waitingFor(labels[kind]):"WAITING FOR QUEST";text(phaseLabel,480,35,18,complete&&!journey.active?"#fff35e":"#fff","center");if(!S.paused)requestAnimationFrame(draw)}
function tone(freq,when,duration,type="square",level=.16,bus=musicMaster){if(!audio||!bus)return;const osc=audio.createOscillator(),gain=audio.createGain();osc.type=type;osc.frequency.setValueAtTime(freq,when);gain.gain.setValueAtTime(level,when);gain.gain.exponentialRampToValueAtTime(.001,when+duration);osc.connect(gain).connect(bus);osc.start(when);osc.stop(when+duration+.03)}
function sweep(from,to,when,duration,level=.35){if(!audio||!sfxMaster)return;const osc=audio.createOscillator(),gain=audio.createGain();osc.type="square";osc.frequency.setValueAtTime(from,when);osc.frequency.exponentialRampToValueAtTime(to,when+duration);gain.gain.setValueAtTime(level,when);gain.gain.exponentialRampToValueAtTime(.001,when+duration);osc.connect(gain).connect(sfxMaster);osc.start(when);osc.stop(when+duration+.03)}
function kick(when){if(!audio||!musicMaster)return;const osc=audio.createOscillator(),gain=audio.createGain();osc.type="sine";osc.frequency.setValueAtTime(118,when);osc.frequency.exponentialRampToValueAtTime(44,when+.12);gain.gain.setValueAtTime(.36,when);gain.gain.exponentialRampToValueAtTime(.001,when+.15);osc.connect(gain).connect(musicMaster);osc.start(when);osc.stop(when+.17)}
function queueMusic(){if(!S.sound||!audio||!musicMaster)return;const start=audio.currentTime+.04,melodies=[[294,370,440,494,440,370,330,415],[330,415,494,587,494,440,370,330],[370,494,554,659,554,494,415,370],[330,440,523,622,523,440,392,294]],bass=[98,110,123,92][musicStep%4],notes=melodies[musicStep%melodies.length];notes.forEach((freq,i)=>tone(freq,start+i*.18,.14,i%4===3?"sawtooth":"square",.12));for(let i=0;i<4;i++){tone(bass,start+i*.36,.3,"triangle",.18);kick(start+i*.36)}musicStep++}
function brickCue(kind){if(!S.sound||!audio||!sfxMaster)return;const now=audio.currentTime,accent={WORKFLOW_TASK:659,BUILD_RESULT:740,CHALLENGE:554,REVISED_RESULT:831,COMPLETE:988}[kind]||659;sweep(190,95,now,.09,.5);tone(accent,now+.045,.08,"square",.48,sfxMaster);tone(accent*1.25,now+.13,.09,"square",.3,sfxMaster)}
function handoffCue(){if(!S.sound||!audio||!sfxMaster)return;const now=audio.currentTime;sweep(260,920,now,.34,.32);tone(988,now+.3,.08,"square",.38,sfxMaster);tone(1319,now+.38,.12,"square",.32,sfxMaster)}
function challengeCue(){if(!S.sound||!audio||!sfxMaster)return;const now=audio.currentTime;tone(466,now,.12,"square",.38,sfxMaster);tone(349,now+.13,.18,"square",.42,sfxMaster)}
function victoryCue(){if(!S.sound||!audio||!sfxMaster)return;const now=audio.currentTime,notes=[392,494,587,740,587,831,988,1175];if(musicMaster){musicMaster.gain.cancelScheduledValues(now);musicMaster.gain.setValueAtTime(Math.max(.03,musicMaster.gain.value),now);musicMaster.gain.exponentialRampToValueAtTime(.035,now+.12)}notes.forEach((freq,i)=>tone(freq,now+i*.17,i===notes.length-1?.75:.14,i%3===2?"sawtooth":"square",.46,sfxMaster));[98,123,147,196].forEach((freq,i)=>tone(freq,now+i*.34,.3,"triangle",.32,sfxMaster));setTimeout(()=>{if(S.sound&&musicMaster&&audio){musicMaster.gain.cancelScheduledValues(audio.currentTime);musicMaster.gain.setValueAtTime(.04,audio.currentTime);musicMaster.gain.exponentialRampToValueAtTime(.22,audio.currentTime+.5)}},1900)}
function cueJourney(journey,kind){if(!journey.active||!S.sound)return;const key=`${journey.index}:${journey.phase}`;if(key===S.cueKey)return;S.cueKey=key;if(journey.phase==="work"){brickCue(kind);if(kind==="CHALLENGE")setTimeout(challengeCue,240)}else if(journey.phase==="handoff"){if(kind==="COMPLETE")victoryCue();else handoffCue()}}
async function startMusic(){if(!audio)return;await audio.resume?.();audioMaster=audio.createDynamicsCompressor();audioMaster.threshold.value=-18;audioMaster.knee.value=12;audioMaster.ratio.value=6;audioMaster.attack.value=.003;audioMaster.release.value=.22;audioMaster.connect(audio.destination);musicMaster=audio.createGain();musicMaster.gain.setValueAtTime(.22,audio.currentTime);musicMaster.connect(audioMaster);sfxMaster=audio.createGain();sfxMaster.gain.setValueAtTime(.72,audio.currentTime);sfxMaster.connect(audioMaster);musicStep=0;queueMusic();musicTimer=setInterval(queueMusic,1440)}
function stopMusic(){if(musicTimer)clearInterval(musicTimer);musicTimer=null;for(const bus of [musicMaster,sfxMaster])if(bus&&audio){bus.gain.cancelScheduledValues(audio.currentTime);bus.gain.setValueAtTime(Math.max(.001,bus.gain.value),audio.currentTime);bus.gain.exponentialRampToValueAtTime(.001,audio.currentTime+.12)}const oldMaster=audioMaster;setTimeout(()=>oldMaster?.disconnect(),180);musicMaster=null;sfxMaster=null;audioMaster=null}
...
function updateHud(data){const c=I18N[lang],snap=data?.snapshot||{},ws=snap.workflows||[],wf=ws[0],ev=wf?.stages||[],last=ev[ev.length-1];document.getElementById("hud-task").textContent=(wf?.task_id||"WAITING").slice(-14);document.getElementById("hud-stage").textContent=`${ev.length} / 5`;document.getElementById("hud-events").textContent=String(ev.length).padStart(3,"0");document.getElementById("observed-at").textContent=snap.observed_at||c.observedWaiting;const key=last?`${wf.task_id}:${last.kind}:${last.ts}`:"";if(key&&S.lastEvent&&key!==S.lastEvent)addFx();S.lastEvent=key}
function updateObservation(info){const c=I18N[lang],badge=document.getElementById("observation-status"),upstream=document.getElementById("upstream-status"),note=document.getElementById("live-note"),allowed=new Set(["ok","degraded","stale","waiting"]),state=allowed.has(info?.status)?info.status:"waiting",last=info?.last_success?formatTime(info.last_success):"",failures=Number(info?.consecutive_failures||0),codes={ok:"↑ OK",degraded:"↑ RETRY",stale:"↑ STALE",waiting:"↑ WAIT"};badge.className="live ok";badge.textContent="● LIVE";badge.title=lang==="zh"?"观察器连接正常":"Viewer connection healthy";upstream.className=`live upstream ${state}`;upstream.textContent=codes[state];upstream.title=state==="ok"?c.upstreamOk:state==="degraded"?c.upstreamRetry(failures):state==="stale"?c.upstreamStale:c.upstreamWait;if(state==="ok"){note.textContent=c.readOk(new Date().toLocaleTimeString());note.style.borderColor="#43d36f"}else if(state==="degraded"){note.textContent=`${c.readDegraded(last)} · ${c.upstreamRetry(failures)}`;note.style.borderColor="#ffd84a"}else if(state==="stale"){note.textContent=c.readStale(last);note.style.borderColor="#ff5b5b"}else{note.textContent=c.readWaiting;note.style.borderColor="#ffd84a"}}
async function refresh(){const c=I18N[lang];try{const res=await fetch(`/atlas.json?v=39&t=${Date.now()}`,{cache:"no-store",headers:{Accept:"application/json","Cache-Control":"no-cache"}});if(!res.ok)throw new Error("HTTP "+res.status);const data=await res.json(),wf=data?.snapshot?.workflows?.[0],workflowKey=wf?`${wf.task_id}:${wf.updated_at}:${(wf.stages||[]).length}`:"";S.data=data;updateHud(data);renderHistory(data);updateObservation(data.observation);if(workflowKey&&workflowKey!==S.workflowKey){S.workflowKey=workflowKey;beginReplay()}}catch(_){const badge=document.getElementById("observation-status"),upstream=document.getElementById("upstream-status"),note=document.getElementById("live-note");badge.className="live stale";badge.textContent=`● ${c.connectionOffline}`;upstream.className="live upstream stale";upstream.textContent="↑ ?";upstream.title=c.upstreamStale;note.textContent=c.readFail;note.style.borderColor="#ff5b5b"}}
...
document.getElementById("sound").addEventListener("click",async e=>{S.sound=!S.sound;if(S.sound){const Audio=window.AudioContext||window.webkitAudioContext;audio=audio||(Audio?new Audio():null);await startMusic();S.cueKey=""}else{stopMusic();S.cueKey=""}e.currentTarget.setAttribute("aria-pressed",String(S.sound));e.currentTarget.textContent=S.sound?"♪ ON":"♪ OFF"});
