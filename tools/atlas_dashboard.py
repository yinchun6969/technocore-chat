...
<title>Technocore Atlas v3.4 Relay Quest</title>
<style>
...
<header class="topbar"><div class="brand"><span class="brand-mark" aria-hidden="true"></span><span class="brand-copy"><b>TECHNOCORE // PIXEL QUEST</b><small id="brand-subtitle">Atlas v3.4 · Agent 接力工作流观察器</small></span></div><div class="top-actions"><button id="language" class="lang-switch" type="button" aria-label="Switch to English">EN</button><div class="live __STATUS_CLASS__">● __STATUS_LABEL__</div></div></header>
...
const S={paused:false,pausedAt:0,sound:false,data:null,lastEvent:"",historyKey:"",workflowKey:"",journey:null,started:performance.now(),replayStart:0,replayCount:0,agents:{Love8:{x:90},Aizong:{x:130},AI2AI:{x:170}},sparkles:[]};
let audio=null;
...
function applyLanguage(next,persist=true){lang=next==="en"?"en":"zh";labels=I18N[lang].labels;actions=I18N[lang].actions;const c=I18N[lang],counts=document.getElementById("footer-counts"),language=document.getElementById("language"),focus=document.getElementById("focus");document.documentElement.lang=lang==="zh"?"zh-CN":"en";document.title=`Technocore Atlas v3.4 ${lang==="zh"?"接力任务":"Relay Quest"}`;document.getElementById("brand-subtitle").textContent=c.subtitle;document.getElementById("game-title").textContent=c.world;document.getElementById("truth-label").textContent=c.truthLabel;document.getElementById("truth-copy").textContent=c.truth;document.getElementById("team-title").textContent=c.team;document.getElementById("workflow-title").textContent=c.workflows;document.getElementById("footer-note").textContent=c.footerNote;document.getElementById("footer-safety").textContent=c.footerSafety;counts.textContent=c.footerCounts(counts.dataset.rooms,counts.dataset.messages,counts.dataset.workflows);language.textContent=lang==="zh"?"EN":"中";language.setAttribute("aria-label",c.switchLabel);focus.title=document.body.classList.contains("focus-mode")?c.focusExit:c.focusEnter;focus.setAttribute("aria-label",focus.title);document.getElementById("replay").textContent=S.replayStart?c.relaying:c.replay;document.getElementById("sound").title=lang==="zh"?"声音默认关闭":"Sound is off by default";for(const card of document.querySelectorAll(".agent-card[data-agent]")){const description=card.querySelector("small");if(description)description.textContent=c.descriptions[card.dataset.agent]||"Agent"}S.historyKey="";updateProgress(S.journey);if(S.data){renderHistory(S.data);document.getElementById("live-note").textContent=c.readOk(new Date().toLocaleTimeString())}else document.getElementById("live-note").textContent=c.initialNote;if(persist)try{localStorage.setItem("atlas-language",lang)}catch(_){}}
function updateProgress(journey){const c=I18N[lang],all=stages(),bar=document.querySelector(".relay-progress"),fill=document.getElementById("progress-fill"),label=document.getElementById("progress-label");let value=0,copy=c.progressWaiting;if(journey?.active&&all.length){value=clamp((journey.index+journey.progress)/all.length,0,1);const phase=journey.phase==="move"?c.phaseMove:journey.phase==="work"?c.phaseWork:c.phaseHandoff;copy=c.progress(journey.index+1,all.length,labels[journey.current?.kind]||journey.current?.kind||c.waitingEvent,phase)}else if(all.some(e=>e.kind==="COMPLETE")){value=1;copy=c.progressDone}else if(all.length){value=clamp(all.length/order.length,0,1);copy=c.waitingSigned}fill.style.transform=`scaleX(${value})`;label.textContent=copy;bar.setAttribute("aria-valuenow",String(Math.round(value*100)));bar.setAttribute("aria-valuetext",copy)}
...
function resetActors(){S.agents.Love8.x=90;S.agents.Aizong.x=130;S.agents.AI2AI.x=170;S.sparkles=[]}
...
function draw(now){const t=(now-S.started)/1000,w=960,h=540,workflow=stage(),journey=relay(now),events=journey.events,current=journey.current,last=events[events.length-1],idx=journey.active?journey.index:last?Math.max(0,order.indexOf(last.kind)):0,kind=current?.kind||last?.kind||order[0],owner=owners[kind],complete=events.some(e=>e.kind==="COMPLETE"),copy=I18N[lang];S.journey=journey;updateProgress(journey);ctx.imageSmoothingEnabled=false;px(0,0,w,h,"#49bdf2");px(0,315,w,125,"#75d8f7");cloud((70+t*1.5)%1050-90,85,1);cloud((540+t*.85)%1100-100,120,.8);hill(-20,410,260,150,"#62c969");hill(170,410,330,205,"#3fac65");hill(560,410,330,180,"#64c96c");pipe(25,350);flag(875,410,complete,t);for(let i=0;i<5;i++){const done=events.some(e=>e.kind===order[i]);block(stageXs[i]-29,350,`${i+1}`,done,i===idx&&journey.active&&!done);text(labels[order[i]],stageXs[i],322,10,"#fff","center");if(done)coin(stageXs[i],292,t*2+i)}px(0,410,w,40,"#8b5a32");for(let x=0;x<w;x+=32){px(x,410,30,18,"#c3843f");px(x+15,430,30,18,"#a86835")}px(0,450,w,90,"#6d3e27");for(let x=0;x<w;x+=48)px(x,475+(x%96?20:0),25,9,"#855031");if(journey.active&&journey.phase==="move"){const a=S.agents[owner];a.x+=(stageXs[idx]-a.x)*.018}for(const name of Object.keys(S.agents)){const motion={walking:journey.active&&name===owner&&journey.phase==="move",action:journey.active&&name===owner&&journey.phase==="work"?kind:""};avatar(name,S.agents[name].x,405,t,motion)}if(journey.active&&journey.phase==="handoff"&&idx<order.length-1){const nextOwner=owners[order[idx+1]];handoff(stageXs[idx],S.agents[nextOwner].x,335,journey.progress,colors[owner])}const hc=["#e2f14a","#f57bd7","#73f0d1","#ff9f43"];for(let i=0;i<10;i++)helper(90+i*82,405,hc[i%4],t,i);bubble((current||last)?.content,clamp(stageXs[idx]-125,12,698),190,250);if(complete&&S.sparkles.length===0)addFx();drawFx(1/60);const phaseLabel=journey.phase==="move"?copy.go(owner):journey.phase==="work"?actions[kind]:journey.phase==="handoff"?(idx<4?copy.handoff(owner,owners[order[idx+1]]):copy.accepted):complete?"QUEST CLEAR!":workflow?copy.waitingSigned:"WAITING FOR QUEST";text(phaseLabel,480,35,18,complete&&!journey.active?"#fff35e":"#fff","center");if(!S.paused)requestAnimationFrame(draw)}
...
function renderHistory(data){const c=I18N[lang],root=document.getElementById("workflow-history"),workflows=data?.snapshot?.workflows||[],key=lang+JSON.stringify(workflows.map(w=>[w.task_id,w.updated_at,(w.stages||[]).length,w.conflicts]));if(key===S.historyKey){renderAgents(workflows);return}S.historyKey=key;if(!workflows.length){const box=make("div","empty"),title=make("b","",c.emptyTitle),copy=make("p","",c.emptyCopy);box.append(title,copy);root.replaceChildren(box);renderAgents([]);return}const fragment=document.createDocumentFragment();workflows.forEach((wf,index)=>{const details=make("details","workflow");details.open=index===0;const summary=make("summary"),identity=make("span"),task=make("b","",wf.task_id||c.unknownTask),updated=make("small","",formatTime(wf.updated_at)),complete=wf.status==="complete",status=make("em",`status ${complete?"complete":"active"}`,complete?c.complete:labels[wf.current_stage]||c.waitingEvent);identity.append(task,updated);summary.append(identity,status);const body=make("div","workflow-body"),quest=make("ol","quest-map"),present=new Set((wf.stages||[]).map(e=>e.kind));quest.setAttribute("aria-label",c.questMap);order.forEach((kind,i)=>{const item=make("li","quest-step"+(present.has(kind)?" done":"")+(kind===wf.current_stage&&!complete?" current":"")),number=make("span","",i+1),info=make("div"),name=make("b","",labels[kind]),owner=make("small","",owners[kind]);info.append(name,owner);item.append(number,info);quest.append(item)});body.append(quest);if(Number(wf.conflicts)>0)body.append(make("p","warning",c.conflicts(wf.conflicts)));const list=make("section","event-list");list.setAttribute("aria-label",c.timeline);for(const ev of (wf.stages||[])){const row=make("article","event-row");row.dataset.agent=ev.agent||"";const icon=make("span","event-icon"),content=make("div"),head=make("header"),name=make("b","",ev.agent||c.unknownAgent),meta=make("span","",`${ev.role||"Agent"} · ${labels[ev.kind]||ev.kind||c.event} · ${c.signedOriginal}`),time=make("time","",formatTime(ev.ts)),copy=make("p","",ev.content||"");icon.setAttribute("aria-hidden","true");head.append(name,meta,time);content.append(head,copy);if(ev.content_truncated)content.append(make("span","truncated",c.truncated));row.append(icon,content);list.append(row)}body.append(list);details.append(summary,body);fragment.append(details)});root.replaceChildren(fragment);renderAgents(workflows)}
...
async function refresh(){const c=I18N[lang];try{const res=await fetch("/atlas.json",{cache:"no-store",headers:{Accept:"application/json"}});if(!res.ok)throw new Error("HTTP "+res.status);const data=await res.json(),wf=data?.snapshot?.workflows?.[0],workflowKey=wf?`${wf.task_id}:${wf.updated_at}:${(wf.stages||[]).length}`:"";S.data=data;updateHud(data);renderHistory(data);if(workflowKey&&workflowKey!==S.workflowKey){S.workflowKey=workflowKey;beginReplay()}document.getElementById("live-note").textContent=c.readOk(new Date().toLocaleTimeString());document.getElementById("live-note").style.borderColor="#43d36f"}catch(_){document.getElementById("live-note").textContent=c.readFail;document.getElementById("live-note").style.borderColor="#ffd84a"}}
